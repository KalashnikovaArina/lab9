...
static class PhongShading
{
    ...
    public static Color ToonShadingModel(Vector3 n, Vector3 l, Color diffColor)
    {
        ...
        if (diff < 0.3)
        {
            clr = MultColor(0.2, diffColor); // Очень темное освещение
        }
        else if (diff < 0.4)
        {
            clr = MultColor(0.4, diffColor); // Темное освещение
        }
        else if (diff < 0.5)
        {
            clr = MultColor(0.5, diffColor); // Темное освещение
        }
        else if (diff < 0.6)
        {
            clr = MultColor(0.6, diffColor); // Среднее освещение
        }
        else if (diff < 0.7)
        {
            clr = MultColor(0.8, diffColor); // Среднее освещение
        }
        else if (diff < 0.8)
        {
            clr = MultColor(0.85, diffColor); // Среднее освещение
        }

        else //(diff < 0.9)
        {
            clr = diffColor; // Яркое освещение
        }
        return clr;
    }
                ...
                if (x1 < x2)
                {
                    for (double cur_x = x1; cur_x <= x2; cur_x += 0.5)
                    {
                        ...
                        if (InLimits((int)cur_x, (int)cur_y, bmp.Width, bmp.Height) && cur_z > z_buff[(int)cur_x, (int)cur_y])
                        {
                            ...
                            Vector3 norm = Interpolate(p, v1, n1, v2, n2, v3, n3); //Нормаль точки интерполируется из нормалей вершин треугольника
...
static class GouraudShading
{
    ...
    {
        Vector3 n2 = n.Normalized(); // Нормаль поверхности в точке, где рассчитывается освещение.
        Vector3 l2 = l.Normalized(); //Направление на источник света.
        double diff = 3 * Math.Max(n2.Dot(l2), 0.0); //диффузное освещение. Math.Max для игнорирования отрицательных значений.
        Color clr;
    ...
    //c : Коэффициент освещения (результат диффузного освещения).
    //color : Исходный цвет материала.
    private static Color MultColor(double c, Color color)
    {
        return Color.FromArgb(Math.Max(0, Math.Min((int)(c * color.R), 255)),
                              Math.Max(0, Math.Min((int)(c * color.G), 255)),
